...
class ExtractParaEvalConfig(MyConfig):
    ...
    def trainTestCalculator(cls):  # 若不局部导入将导致与其他模块冲突
        from .processors import TrainTestCalculator
    ...
    def resultEvaluation(cls):  # 采用set inject
        from .processors import ResultEvaluation
    ...
    @classmethod
    def mlModel(cls):
        para = dict(n_estimators=500, max_depth=6, max_features=0.2,class_weight="balanced",
    ...
    def trainAndTest(cls):
        from .processors import TrainAndTest
        processor = TrainAndTest(dependencies=[MyNode.DOE2.value],reset=True)
        props = MyProperties()
    ...
    def extractParaEvalServer(cls):
        ...
        server = MsgServer(cls.extractParaEvalApp(), n_jobs=None, single_thread=True, time_out=10)
